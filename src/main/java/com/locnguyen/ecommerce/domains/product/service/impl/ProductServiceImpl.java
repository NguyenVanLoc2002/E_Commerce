package com.locnguyen.ecommerce.domains.product.service.impl;
...
import com.locnguyen.ecommerce.domains.product.service.ProductSearchTextBuilder;
...
public class ProductServiceImpl implements ProductService {
    ...
    public PagedResponse<ProductListItemResponse> getPublishedProducts(ProductFilter filter,
        ...
        Page<Product> page = searchProducts(publicFilter, pageable);
    ...
    public ProductDetailResponse updateProduct(UUID id, UpdateProductRequest request) {
        ...
        boolean searchableChanged = false;
    ...
    public PagedResponse<ProductListItemResponse> getAllProducts(ProductFilter filter,
                                                                 Pageable pageable) {
        Page<Product> page = searchProducts(filter, pageable);
        return PagedResponse.of(page.map(productMapper::toListItem));
    }

    /**
     * Routes product list queries:
     * <ul>
     *   <li>blank keyword → JPA Specification + EntityGraph (existing behavior).</li>
     *   <li>non-blank keyword → MariaDB FULLTEXT (relevance-ordered).</li>
     * </ul>
     * Public query params, pagination and response format are unchanged.
     */
    private Page<Product> searchProducts(ProductFilter filter, Pageable pageable) {
        if (filter == null || !StringUtils.hasText(filter.getKeyword())) {
            return productRepository.findAll(
                    ProductSpecification.withFilter(filter), pageable);
        }

        String normalized = SearchTextNormalizer.normalize(filter.getKeyword());
        if (normalized.isEmpty()) {
            return productRepository.findAll(
                    ProductSpecification.withFilter(filter), pageable);
        }

        return productRepository.searchByKeyword(toBooleanModeKeyword(normalized), filter, pageable);
    }

    /**
     * Convert a normalized keyword into a MariaDB BOOLEAN MODE expression:
     * each term becomes a {@code +term*} so that all terms must match and
     * prefix-matching is enabled. Reserved BOOLEAN MODE operators are stripped
     * to avoid syntax errors from raw user input.
     */
    private static String toBooleanModeKeyword(String normalized) {
